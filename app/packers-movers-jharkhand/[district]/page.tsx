...
const districtPopularRoutes: Record<string, string[]> = {
  ...
  'west-singhbhum': [
    ...
    "West Singhbhum → Ranchi",
    "West Singhbhum → Simdega",
    "West Singhbhum → Ghatshila",
  ],
  'east-singhbhum': [
    "East Singhbhum → Jamshedpur",
    "East Singhbhum → Ghatshila",
    "East Singhbhum → Ranchi",
    "East Singhbhum → West Singhbhum",
    "East Singhbhum → Chaibasa",
  ],
};
...
function getDistrictProfile(districtName: string, slug: string): DistrictProfile {
  const profiles: Record<string, DistrictConfig> = {
      ...
      faqs: [
        {
          ...
          answer:
            "Yes, we support Ranchi city moves with trained packing, loading, transport, and careful unloading at the destination.",
        },
        {
          question: "Can you handle office relocation in Ranchi?",
          answer:
            "Yes, we can plan office relocation in Ranchi with minimal downtime and proper handling of files, devices, and furniture.",
        },
        {
    ...
    bokaro: {
      introCopy:
        "We offer Bokaro packers and movers support for household shifting, office moves, vehicle transport, and loading with safe handling and route planning.",
    ...
    ramgarh: {
      introCopy:
        "We provide Ramgarh packers and movers service for local shifting, office relocation, vehicle transport, and loading with careful planning and verified manpower.",
    ...
    hazaribagh: {
      introCopy:
        "We support Hazaribagh moves with household packing, office relocation, storage coordination, and safe transport across local areas and nearby districts.",
    ...
    dhanbad: {
      introCopy:
        "We support packers and movers work in Dhanbad with house shifting, office relocation, vehicle transport, and loading help across the city and nearby coal-belt routes.",
    ...
    jamshedpur: {
      introCopy:
        "For Jamshedpur, we arrange local and intercity shifting with careful packing across major commercial and residential belts of the steel city.",
    ...
    deoghar: {
      introCopy:
        "We handle Deoghar packers and movers service for household relocation, office shifting, and vehicle transport around the temple town and nearby routes.",
    ...
    dumka: {
      introCopy:
        "In Dumka, we provide smooth packing and moving support with local pickup planning for homes, shops, and office movement across the district.",
    ...
    giridih: {
      introCopy:
        "We provide Giridih packers and movers service with careful packing, loading, and transport across town and nearby market belts.",
    ...
    godda: {
      introCopy:
        "Godda moves are handled with district-wise route planning for homes, offices, and vehicle transport across the town and rural service points.",
    ...
    gumla: {
      introCopy:
        "In Gumla, our packers and movers team supports residential shifting, office moves, and loading work with route planning for district and nearby village belts.",
    ...
    chatra: {
      introCopy:
        "Chatra packers and movers support is available for local shifting, office relocation, and vehicle movement with planning around town and connecting road routes.",
    ...
    garhwa: {
      introCopy:
        "We offer Garhwa packers and movers service for household shifting and office relocation with reliable support for town and nearby transport routes.",
    ...
    jamtara: {
      introCopy:
        "Jamtara moves are planned with local and intercity support for households, offices, loading, and vehicle shifting across the district.",
    ...
    khunti: {
      introCopy:
        "We support Khunti packers and movers work for local shifting, office movement, and transport planning across the town and nearby routes.",
    ...
    koderma: {
      introCopy:
        "Koderma packers and movers service covers home shifting, office relocation, and vehicle transport with safe loading across town and nearby belts.",
    ...
    latehar: {
      introCopy:
        "Latehar shifting support is available for homes, offices, and vehicle transport with careful route planning for the district and forest-side routes.",
    ...
    lohardaga: {
      introCopy:
        "We provide Lohardaga packers and movers support for residential shifting and office relocation with local route planning and quick support.",
    ...
    pakur: {
      introCopy:
        "Pakur packers and movers service is ideal for local shifting, loading, and vehicle movement across town, railway-side pockets, and nearby routes.",
    ...
    palamu: {
      introCopy:
        "Palamu moves are handled with support for Medininagar and nearby towns, including household shifting, office relocation, and vehicle transport.",
    ...
    sahebganj: {
      introCopy:
        "Sahebganj packers and movers support covers the river-town belt, market areas, and nearby pickup points for home and office moves.",
    ...
    "seraikela-kharsawan": {
      introCopy:
        "Seraikela Kharsawan shifting service is arranged across Seraikela, Kharsawan, Chandil, Gamharia, and nearby industrial routes.",
    ...
    simdega: {
      introCopy:
        "Simdega packers and movers support covers the district town and surrounding blocks with safe packing, loading, and route coordination.",
    ...
    "west-singhbhum": {
      introCopy:
        "West Singhbhum moves are supported across Chaibasa and nearby mining and forest-side routes with careful packing and planned dispatch.",
    ...
    "east-singhbhum": {
      introCopy:
        "East Singhbhum shifting support covers Jamshedpur city and surrounding blocks with local packing, transport, and vehicle delivery planning.",
  ...
  if (!profile) {
    return buildDistrictProfile(districtName, slug, {
      localities: [
        ...
        `${districtName} market area`,
        `${districtName} residential belt`,
        `${districtName} bus stand`,
      ],
...
export function generateStaticParams() {
  return jharkhandDistricts.map((district) => ({
    district: district.slug,
  }));
}

export function generateMetadata({ params }: { params: Promise<DistrictPageParams> }): Promise<Metadata> {
  return params.then(({ district }) => {
    const districtData = getDistrictBySlug(district);

    if (!districtData) {
      return {
        title: "Packers and Movers in Jharkhand",
      };
    }

    return {
      title: `Packers and Movers in ${districtData.name} | Soni Packers and Movers`,
...
export default async function DistrictPage({ params }: { params: Promise<DistrictPageParams> }) {
