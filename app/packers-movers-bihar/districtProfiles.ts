...
function buildDefaultPopularRoutes(districtName: string, localities: string[], nearbyDistricts: string[]) {
  const primaryLocality = localities[0] ?? districtName;
  const secondaryLocality = localities[1] ?? primaryLocality;
  const firstNearby = nearbyDistricts[0] ?? "Patna";
  const secondNearby = nearbyDistricts[1] ?? firstNearby;
  const thirdNearby = nearbyDistricts[2] ?? firstNearby;

  return [
    `${districtName} → ${firstNearby}`,
    `${districtName} Railway Station → ${primaryLocality}`,
    `${districtName} Bus Stand → ${secondaryLocality}`,
    `${primaryLocality} → ${secondNearby}`,
    `${districtName} → ${thirdNearby}`,
  ];
}

function generatePincodeSeries(basePin: string, count = 8) {
  const start = Number.parseInt(basePin, 10);

  if (Number.isNaN(start)) {
    return [];
  }

  return Array.from({ length: count }, (_, index) => String(start + index).padStart(6, "0"));
}

function buildDefaultPincodeCoverage(districtName: string, slug: string) {
  const seed = `${districtName}:${slug}`.split("").reduce((acc, char) => acc + char.charCodeAt(0), 0);
  const basePin = biharPincodeSeeds[seed % biharPincodeSeeds.length];

  return generatePincodeSeries(basePin);
}
...
function buildDistrictProfile(districtName: string, slug: string, config: DistrictConfig): DistrictProfile {
  return {
    ...
    overviewParagraphs: [
      ...
      `As one of the <strong>top-rated packers and movers near ${districtName}</strong>, we help families and businesses relocate locally and across Bihar without hassle.`,
    ...
    popularRoutes: config.popularRoutes ?? districtPopularRoutes[slug] ?? buildDefaultPopularRoutes(districtName, config.localities, config.nearbyDistricts),
    pincodeCoverage: config.pincodeCoverage ?? districtPincodes[slug] ?? buildDefaultPincodeCoverage(districtName, slug),
...
export function getLocalityProfile(districtName: string, districtSlug: string, localityName: string): LocalityProfile {
  ...
  const popularRoutes = [
    ...
    `${localityName} → ${districtProfile.nearbyDistricts[0] ?? "Patna"}`,
    `${localityName} → ${districtProfile.nearbyDistricts[1] ?? "Gaya"}`,
  ].map((route) => route.trim());

  return {
    areaDisplayName: `${localityName}, ${districtName}`,
    mapQuery: `${localityName}, ${districtName}, Bihar`,
...
export function getDistrictProfile(districtName: string, slug: string): DistrictProfile {
  const profiles: Record<string, DistrictConfig> = {
    patna: {
      introCopy:
        "We provide home shifting, office relocation, vehicle transport, and loading support across Patna — including Boring Road, Kankarbagh, Bailey Road, Danapur, and nearby routes.",
      localities: ["Boring Road", "Kankarbagh", "Bailey Road", "Danapur", "Patliputra", "Phulwari Sharif", "Exhibition Road", "Ashok Rajpath"],
      nearbyDistricts: ["Nalanda", "Vaishali", "Bhojpur", "Jehanabad"],
      heroImage: siteAssets.common.movingTeamOne,
      faqs: [
        {
          question: "Do you provide packing and moving in Patna city?",
          answer: "Yes, we support Patna city moves with trained packing, loading, transport, and careful unloading at the destination.",
        },
        {
          question: "Can you handle office relocation in Patna?",
          answer: "Yes, we can plan office relocation in Patna with minimal downtime and proper handling of files, devices, and furniture.",
        },
        {
          question: "Do you serve nearby Patna localities too?",
          answer: "Yes, we work across nearby Patna localities and also help with intercity moves from the city.",
        },
        {
    ...
    gaya: {
      introCopy: "Gaya packers and movers support for household shifting, office moves, and vehicle transport with safe handling across the city and Bodh Gaya routes.",
      localities: ["Gaya town", "Bodh Gaya", "Tekari", "Sherghati", "Wazirganj", "Barachatti"],
      nearbyDistricts: ["Nawada", "Jehanabad", "Aurangabad", "Patna"],
      heroImage: siteAssets.services.office.faqSlideOne,
    },
    muzaffarpur: {
      introCopy: "Muzaffarpur packers and movers service for local shifting, office relocation, and loading with route planning across town and Saraiya belts.",
      localities: ["Muzaffarpur town", "Saraiya", "Kanti", "Motipur", "Bochaha", "Kurhani"],
      nearbyDistricts: ["Sitamarhi", "Vaishali", "Samastipur", "East Champaran"],
      heroImage: siteAssets.services.vehicle.cardImage,
    },
    bhagalpur: {
      introCopy: "Bhagalpur shifting support for homes, offices, and vehicles with careful packing across the city, Sabour, and river-side routes.",
      localities: ["Bhagalpur town", "Sabour", "Kahalgaon", "Naugachia", "Sultanganj", "Colgong"],
      nearbyDistricts: ["Banka", "Munger", "Khagaria", "Katihar"],
  ...
  if (!profile) {
    return buildDistrictProfile(districtName, slug, {
      ...
      nearbyDistricts: ["Patna", "Gaya", "Muzaffarpur", "Bhagalpur"],
      heroImage: siteAssets.common.districtDispatchPlanning,
    });
  }
